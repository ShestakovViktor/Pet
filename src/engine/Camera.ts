import {ml} from './math';
import {Vector2, Vector3, Matrix4} from '@engine/type';
import {Gl} from './gl';
...
export class Camera {
	...
	private position = ml.vec3.init(7, 7, 7);
	...
	*/
	...
	getProjectionMatrix(): Matrix4 {
		const aspect = this.gl.getAspectRatio();
		const result = ml.mat4.init();

		if (this.projectionMode == 'perspective') {
			ml.perspective(result, 45, aspect, 0.1, 100.0);
		}
		else {
